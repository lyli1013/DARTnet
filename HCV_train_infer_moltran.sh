...
# Passes MolFormer args so missing *_emb.pt are generated via conda env MolTran_CUDA11.
# Requires: preprocessing/checkpoints/N-Step-Checkpoint_3_30000.ckpt and env MolTran_CUDA11.
# If precomputed embeddings already exist under dataset_HCV/, prefer HCV_train_infer.sh.

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT_ROOT="${DART_OUTPUT_ROOT:-${PROJECT_ROOT}/outputs}"
LOG_ROOT="${DART_LOG_ROOT:-${PROJECT_ROOT}/logs}"

MOLFORMER_CKPT="${MOLFORMER_CKPT:-${PROJECT_ROOT}/preprocessing/checkpoints/N-Step-Checkpoint_3_30000.ckpt}"
MOLFORMER_ENV="${MOLFORMER_ENV:-MolTran_CUDA11}"
